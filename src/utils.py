...
class Block:
    ...
    """
    def __init__(self, data, node_id, previous_block_hash, NONCE_MAX_VALUE):
        ...
        self.NONCE_MAX_VALUE = NONCE_MAX_VALUE
        self.data = data
    ...
    def proof_of_work(self, k):
        ...
        while lhs[:k] != rhs:
            self.nonce += 1
            if self.nonce > self.NONCE_MAX_VALUE:
                return False
...
class BlockChain:
    ...
    """
    def __init__(self, NONCE_MAX_VALUE):
        self.chain = []
        res, block = self.add_block("First Block", "-1", "0", 4, NONCE_MAX_VALUE)
        if res:
            self.chain.append(block)

    def add_block(self, data, node_id, previous_block_hash, k, NONCE_MAX_VALUE):
        ...
        """
        block = Block(data, node_id, previous_block_hash, NONCE_MAX_VALUE)
...
class Node:
    ...
    def propose_block(self, data, k, NONCE_MAX_VALUE):
        ...
        res, _ = self.blockchain.add_block(data, self.id, previous_block_hash, k, NONCE_MAX_VALUE)
        return res
        # return True